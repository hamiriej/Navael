...
import { useLabOrders, type LabOrder } from "@/contexts/lab-order-context";
import { useInvoices } from "@/contexts/invoice-context"; // Import useInvoices

const LAB_TEST_PRICES_STORAGE_KEY = 'navael_lab_test_prices';
...
export default function OrderLabTestPage() {
  ...
  useEffect(() => {
    ...
    try {
      const storedLabTestPrices = localStorage.getItem(LAB_TEST_PRICES_STORAGE_KEY);
      if (storedLabTestPrices) {
        setAvailableLabTests(JSON.parse(storedLabTestPrices));
      } else {
        setAvailableLabTests([]); 
      }
    } catch (e) {
      console.error("Error loading lab test prices for order page:", e);
      setAvailableLabTests([]);
    }
  ...
  useEffect(() => {
    if (patientIdFromQuery) {
      const foundPatient = getPatientById(patientIdFromQuery);
      if (foundPatient) {
  ...
  const onSubmit = async (values: LabOrderFormValues) => {
    ...
    const testsForOrder: LabTest[] = values.selectedTests.map((testName, index): LabTest => {
        ...
        return {
          id: testDefinition ? `${testDefinition.id}_${Date.now()}` : `custom_${index}_${Date.now()}`,
          name: testName,
    ...
    if (totalOrderPrice > 0) {
        ...
        try {
            const createdInv = await createInvoice(invoiceData); // Use context
    ...
    const newLabOrderData: Omit<LabOrder, 'id'> = {
      ...
      invoiceId: invoiceId,
      paymentStatus: totalOrderPrice > 0 ? "Pending Payment" : "N/A",
    };
    ...
    try {
      ...
      console.log("Lab Order Data (from context):", createdLabOrder);
