...
export default function EditConsultationNotePage() {
  ...
  useEffect(() => {
    const loadNote = async () => {
      if (consultationId) {
        ...
        const note = await fetchConsultationById(consultationId);
        if (note) {
  ...
  const handleQuickAction = async (targetHref: string) => {
    if (!consultationNote) {
      toast({ title: "Error", description: "No consultation note loaded to link actions.", variant: "destructive" });
      return;
    }
    const finalPatientId = consultationNote.patientId;
    ...
    if (saved) {
      const consultationIdForQuery = consultationNote.id;
      const appointmentIdForQuery = consultationNote.linkedAppointmentId;
      ...
      <Card className="max-w-4xl mx-auto shadow-lg">
        <CardHeader>
          ...
          <CardDescription>
            ...
            {consultationNote?.linkedAppointmentId && (
              <span className="block text-sm text-muted-foreground mt-1">
                (Linked to Appointment ID: {consultationNote.linkedAppointmentId})
              </span>
            )}
          </CardDescription>
        </CardHeader>
              ...
              {currentPatientIdForActions && (
                <div className="space-y-3 pt-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Button type="button" variant="outline" size="sm" onClick={() => handleQuickAction(`/dashboard/pharmacy/prescribe`)} disabled={isSubmitting || isLoadingNote || isContextLoading}>
                      <Pill className="mr-2 h-4 w-4" /> Prescribe Medication
                    </Button>
                    <Button type="button" variant="outline" size="sm" onClick={() => handleQuickAction(`/dashboard/lab/order`)} disabled={isSubmitting || isLoadingNote || isContextLoading}>
                      <FlaskConical className="mr-2 h-4 w-4" /> Order Lab Test
                    </Button>
                    <Button type="button" variant="outline" size="sm" onClick={() => handleQuickAction(`/dashboard/appointments/new`)} disabled={isSubmitting || isLoadingNote || isContextLoading}>
