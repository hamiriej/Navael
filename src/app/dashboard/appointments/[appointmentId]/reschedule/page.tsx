...
import { CalendarClock, ArrowLeft, Loader2, Save, CalendarIcon } from "lucide-react";
...
export default function RescheduleAppointmentPage() {
  ...
  const { fetchAppointmentById, updateAppointment, isLoadingAppointments } = useAppointments();
  ...
  const onSubmit = async (values: RescheduleFormValues) => {
    ...
    try {
      const updatedAppt = await updateAppointment(appointmentId, updatedAppointmentData);
      if (updatedAppt) {
        logActivity({
